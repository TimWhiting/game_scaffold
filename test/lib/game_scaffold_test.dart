...
void testGame<E extends Event, T extends Game>(
  ...
  darttest.test(testName, () async {
    ...
    for (final p in players) {
      ref[p.id] = ProviderContainer(parent: root, overrides: [currentPlayerIDProvider.overrideWithValue(p.id)]);
...
class GameTester<E extends Event, T extends Game> {
  ...
  void event(PlayerEvent<E> event, Function(GameState<E, T>, GameError?) outcome) {
    backendContainer.read(BackendProviders.engine.notifier).handleEvent(event);
  ...
  GameState<E, T> get game => backendContainer.read(BackendProviders.state.notifier) as GameState<E, T>;
